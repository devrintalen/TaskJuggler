= Interactive Charts for TaskJuggler

TaskJuggler is a great tool. The static image charts that it produces,
however, are not as good as the rest of it. I wanted to fix that.

This branch adds a new +htmljs+ report format that renders
TaskJuggler's full scheduling data as an interactive Gantt chart in
the HTML output. All the same data is supported, and the interactive
chart allows you to see both the big picture and the details without
needing to regenerate the report every time.

link:screenshots/overview.png
<br /><em>(The interactive Gantt chart, showing tasks, dependency arrows, and resource
load. Add <tt>formats htmljs</tt> to any taskreport or resourcereport to get this.)</em>

== What's New

=== Pan and Zoom

The timeline is fully interactive. Scroll to zoom in and out — from a
year-at-a-glance view all the way down to individual days — and drag to pan
horizontally through the project. The chart header stays fixed as you scroll
vertically, so you always know where you are in time.

link:screenshots/pan-zoom.gif
<br /><em>(Panning and zooming through a multi-month project schedule.)</em>

The chart automatically adjusts its level of detail as you zoom: year and
quarter scales show summarized resource load bars, week and day scales reveal
individual task durations, time-off zones, and hourly grid lines. Transitions
between levels are animated so the chart never feels like it's jumping around.

link:screenshots/lod-zoom.gif
<br /><em>(Zooming from year scale down to day scale, showing automatic level-of-detail
transitions.)</em>

=== Hover Highlighting

Hovering over any row highlights it across both the left panel and the Gantt
chart simultaneously. Hover directly over a task bar and the bar itself lights
up with an orange border and its dependency arrows are highlighted in orange —
making it easy to trace the critical path on a busy chart.

link:screenshots/hover-highlight.gif
<br /><em>(Hovering over a task bar highlights the bar and its dependency arrows.)</em>

=== Tooltips

Hovering over a task bar shows a tooltip with the task's start date, end date,
duration, and completion percentage, styled to match the static HTML report.

link:screenshots/tooltip.png
<br /><em>(Tooltip popup on a task bar showing scheduling details.)</em>

=== Off-Duty Zones

Weekends and holidays defined in your project are shaded on the timeline, just
like in the static HTML report. They fade out at lower zoom levels where they'd
be too narrow to be meaningful.

== How to Use It

Add +formats htmljs+ to any +taskreport+ or +resourcereport+ in your project
file:
...
Then generate reports as usual with +tj3+. The output is a self-contained HTML
file with all JavaScript and CSS inlined — no external dependencies, no server
required.

The +htmljs+ format can also be used inside a +textreport+, the same way the
static HTML format works today.

== Thanks

Finally, a thanks to all the developers and contributors of TaskJuggler
for all their work. If the features in this branch are useful at all to you,
consider contributing back to their efforts.
