...
class TaskJuggler
  ...
  class TableReport < ReportBase
    ...
    # Turn the TableReport into a self-contained interactive Gantt chart page.
    # Returns an Array of XMLElement objects (same convention as to_html).
    def to_htmljs
      html = []
      ...
      gantt = chart_col ? chart_col.column&.cell1&.special : nil

      chart_start = (gantt ? gantt.start : nil) || @project['start']
      chart_end   = (gantt ? gantt.end   : nil) || @project['end']

      scenarios      = a('scenarios')
      scenario_names = scenarios.map { |idx| @project.scenario(idx).id }
      now_date       = @project['now'] || TjTime.new

      col_defs = (a('columns') || []).reject { |c|
      ...
      project_data = {
        'start'        => chart_start ? chart_start.strftime('%Y-%m-%d') : nil,
        'end'          => chart_end   ? chart_end.strftime('%Y-%m-%d')   : nil,
      ...
      base_query = @project.reportContexts.last.query.dup
      rows_json  = build_htmljs_rows(gantt, scenarios, base_query,
                                     chart_start, chart_end)
      gantt_data = { 'project' => project_data, 'rows' => rows_json }
      json_str   = htmljs_to_json(gantt_data)

      # ── Inline D3.js ────────────────────────────────────────────────────────
      d3_src = htmljs_find_data_file('data/js/d3.min.js')
      if d3_src
  ...
  private
    ...
    # ── Interactive chart (to_htmljs) helpers ──────────────────────────────────

    # Build the rows JSON array from the intermediate ReportTable.
    def build_htmljs_rows(gantt, scenarios, base_query, chart_start, chart_end)
      return [] unless @table
      ...
      @table.each_line do |line|
        property   = line.property
        scope_prop = line.scopeLine ? line.scopeLine.property : nil
        key        = [property.object_id, scope_prop&.object_id]
        next if seen[key]
        seen[key] = true

        if property.is_a?(Task) && scope_prop.nil?
          rows << build_task_row_js(property, scenarios, gantt, base_query,
                                    chart_start, chart_end)
        elsif property.is_a?(Resource) && scope_prop.nil?
          resource_no += 1
          rows << build_resource_row_js(property, scenarios, gantt, base_query,
                                        chart_start, chart_end, resource_no)
        elsif property.is_a?(Resource) && scope_prop.is_a?(Task)
          rows << build_nested_resource_row_js(property, scope_prop, scenarios,
                                               gantt, base_query,
                                               chart_start, chart_end)
        elsif property.is_a?(Task) && scope_prop.is_a?(Resource)
          rows << build_nested_task_row_js(property, scope_prop, scenarios,
                                           gantt, base_query,
                                           chart_start, chart_end)
        end
      end
    ...
    def build_task_row_js(task, scenarios, gantt, base_query,
                          chart_start, chart_end)
      scenarios_data = {}
      scenarios.each do |idx|
        ...
        if gantt_line
          ...
          complete = htmljs_query_num(base_query, task, 'complete', idx)
        end
        ...
        scenarios_data[sc_id] = {
          ...
          'effort'    => htmljs_query_str(base_query, task, 'effort',  idx),
          'cost'      => htmljs_query_str(base_query, task, 'cost',    idx),
          'revenue'   => htmljs_query_str(base_query, task, 'revenue', idx),
          'timeoff'   => timeoff
        }
      end
    ...
    def build_resource_row_js(resource, scenarios, gantt, base_query,
                               chart_start, chart_end, no)
      load_data  = {}
      col_defs   = (a('columns') || []).reject { |c|
        c.id == 'chart' || c.id == 'weekly' || c.id == 'no' || c.id == 'name'
      }
      ...
      scenarios.each do |idx|
        ...
        if gantt_line
          ...
          load_data[sc_id] = htmljs_load_buckets(resource, nil, idx,
                                                  ['busy', 'free'],
                                                  chart_start, chart_end)
        end
      end
      ...
      col_defs.each do |col|
        cols_data[col.id] = htmljs_query_str(base_query, resource, col.id,
                                             scenarios.first || 0)
      end
    ...
    def build_nested_resource_row_js(resource, task, scenarios, gantt,
                                      base_query, chart_start, chart_end)
      load_data = {}
      scenarios.each do |idx|
        sc_id      = @project.scenario(idx).id
        ...
        if gantt_line
          ...
          load_data[sc_id] = htmljs_load_buckets(resource, task, idx,
                                                  ['assigned', 'busy', 'free'],
                                                  chart_start, chart_end)
        end
      end
    ...
    def build_nested_task_row_js(task, resource, scenarios, gantt,
                                  base_query, chart_start, chart_end)
      load_data = {}
      scenarios.each do |idx|
        sc_id      = @project.scenario(idx).id
        ...
        if gantt_line
          ...
          load_data[sc_id] = htmljs_load_buckets(task, resource, idx,
                                                  ['busy'],
                                                  chart_start, chart_end)
        end
      end
    ...
    # (e.g. 'weekly' column reports).  Implements the same task-interval
    # clipping as GanttLine#generateResource.
    def htmljs_load_buckets(property, scope_property, sc_idx, categories,
      ...
      while day < chart_end
        ...
        si = @project.dateToIdx(eff_start)
        ei = @project.dateToIdx(eff_end)
    ...
    # Collect time-off intervals for a property as [ [startUnix, endUnix], ... ].
    ...
    # Run a query and return the numeric result.
    ...
    # Minimal JSON serialiser — avoids a hard dependency on the json gem.
    def htmljs_to_json(obj)
      case obj
      when Hash
        pairs = obj.map { |k, v| "#{htmljs_to_json(k.to_s)}:#{htmljs_to_json(v)}" }
        "{#{pairs.join(',')}}"
      when Array
        "[#{obj.map { |v| htmljs_to_json(v) }.join(',')}]"
      when String
        "\"#{obj.gsub('\\', '\\\\\\\\').gsub('"', '\\"').gsub("\n", '\\n')}\""
      when Integer, Float
        obj.to_s
